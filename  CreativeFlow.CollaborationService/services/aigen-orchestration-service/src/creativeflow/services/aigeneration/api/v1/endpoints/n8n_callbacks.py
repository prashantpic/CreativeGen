...
async def handle_n8n_final_generation_callback(
    ...
    """
    Webhook endpoint for n8n to post the result of a final asset generation job.
    """
...
async def handle_n8n_error_callback(
    ...
    """
    Webhook endpoint for n8n to report an error that occurred during a generation job.
    """
    logger.warning(f"Received error callback for request_id: {callback_payload.generation_request_id}, message: {callback_payload.error_message}")
